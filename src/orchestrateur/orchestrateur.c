...
void load_agents(const char *filename) {
    ...
    if(!file){ perror("Error opening agents file"); exit(1); }
    agent_count = 0;
    while(agent_count < MAX_AGENTS && fscanf(file, "%15s %d", agents[agent_count].ip, &agents[agent_count].port)==2)
        agent_count++;
    fclose(file);
}
...
void *send_command(void *arg) {
    ...
    if(connect(sockfd, (struct sockaddr*)&addr, sizeof(addr)) < 0){ close(sockfd); pthread_exit(NULL); }
...
int main(int argc, char *argv[]){
    if(argc < 2){ printf("Usage: %s <agents.conf>\n", argv[0]); exit(1); }
    load_agents(argv[1]);
    display_ascii();
    display_menu();
    char input[512];
    while(1){
        ...
        if(strcmp(input,"help")==0)
            ...
            printf("---- Summary ----\n");
            for(int i=0;i<agent_count;i++){
                printf("Agent %s:%d summary:\n%s\n", tasks[i].agent.ip, tasks[i].agent.port, summarize_response(tasks[i].response));
            }
