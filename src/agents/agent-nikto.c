...
int main(int argc, char *argv[]){
    ...
    if(strcmp(token, "SCAN") == 0){
        ...
        char *mode = strtok(NULL, " \t\n");
        if(target==NULL || port_str==NULL || interval_str==NULL || mode==NULL){ char msg[]="Usage: POLL <target> <port> <interval> <mode>\n"; write(client_fd, msg, strlen(msg)); }
        else {
            ...
            for(int i=0;i<3;i++){ execute_and_send(nikto_cmd, client_fd); sleep(interval); }
        }
