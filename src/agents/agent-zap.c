...
// TODO: Consider implementing error logging mechanism
// TODO: Add timeout mechanism for long-running scans
// TODO: Consider implementing a more robust command parser

/*
 * Executes a shell command and streams its output to the client
 * Uses popen() for command execution which creates a pipe between the calling
 * process and the executed command
 */
void execute_and_send(const char *cmd, int client_fd) {
    ...
    // Stream output back to client as it becomes available
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(token, "SCAN") == 0) {
        ...
        if (target == NULL || mode == NULL) {
            ...
            char zap_cmd[1024];
            // Construct ZAP command based on mode
    ...
    // Handle POLL command - performs periodic scans
    else if (strcmp(token, "POLL") == 0) {
        char *target = strtok(NULL, " \t\n");
        char *interval_str = strtok(NULL, " \t\n");
        char *mode = strtok(NULL, " \t\n");
        
        if (target == NULL || interval_str == NULL || mode == NULL) {
            char msg[] = "Usage: POLL <target> <interval> <mode>\n";
            write(client_fd, msg, strlen(msg));
        } else {
